...
install_dependencies() {
    ...
    apt-get update
    apt-get install -y iproute2 nftables systemd frr dhcp-server mc wget openssh-server sudo iputils-ping
...
# IPs для проверки интерфейсов
PING_IP_ISP="$DEFAULT_GW"          # 172.16.4.1 - шлюз ISP
PING_IP_VLAN_SRV="192.168.10.2"    # Предполагаемый IP HQ-SRV
PING_IP_VLAN_CLI="192.168.20.10"   # Начало диапазона DHCP для CLI
PING_IP_VLAN_MGMT="192.168.99.2"   # Предполагаемый IP для управления
TUNNEL_REMOTE_TUNNEL_IP="172.16.100.2"  # Удалённый конец GRE-туннеля
PING_IP_TUNNEL="$TUNNEL_REMOTE_TUNNEL_IP"
...
check_interfaces() {
    echo "Проверка подключения интерфейсов..."
    
    # Список интерфейсов и соответствующих IP для пинга
    declare -A interfaces
    interfaces["$INTERFACE_ISP"]="$PING_IP_ISP"
    interfaces["$INTERFACE_VLAN_SRV"]="$PING_IP_VLAN_SRV"
    interfaces["$INTERFACE_VLAN_CLI"]="$PING_IP_VLAN_CLI"
    interfaces["$INTERFACE_VLAN_MGMT"]="$PING_IP_VLAN_MGMT"
    interfaces["$TUNNEL_NAME"]="$PING_IP_TUNNEL"
    
    for iface in "${!interfaces[@]}"; do
        ping_ip="${interfaces[$iface]}"
        if ping -I "$iface" -c 1 -W 1 "$ping_ip" &> /dev/null; then
            status="\e[32;5mOK\e[0m"  # Зелёный мигающий текст
        else
            status="\e[31;5mFAIL\e[0m"  # Красный мигающий текст
        fi
        echo -e "$iface: $status"
    done
}

# Функция отображения сетевой топологии в ASCII
display_network_map() {
    echo "Сетевая топология (Рисунок 1):"
    echo ""
    echo "                  [Internet]"
    echo "                      |"
    echo "                  ens192"
...
while true; do
    ...
    echo "12. Отобразить сетевую карту и проверить интерфейсы"
    echo "0. Выход"
